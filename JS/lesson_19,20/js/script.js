...
// 1 Task
  ...
  Skills = _.sortBy(_.uniq(Skills), function (skill) {
            return skill.toLowerCase();
        });
  console.log('Array of sorted skills: ',Skills);

// 2 Task

  var namesAndFriends = [];

  function Friend (name, friends) {
    this.friends = friends;
    this.name = name;
  }

  _.forEach(data, function(value){
    var count = _.size(value.friends);
    var name = value.name;
    var data = new Friend(name, count);
    namesAndFriends.push(data);
  });

  var result = _.orderBy(namesAndFriends, ['friends'], ['asc']);
  var orderedFriends = _.map(result, 'name');

  console.log('Array of names: ',orderedFriends);


// 3 Task

  var friendList = _.map(data, 'friends');
  friendList = _.flatten(friendList, ',');
  friendList = _.map(friendList, 'name');
  friendList = _.uniq(friendList, 'name');
  
  console.log('Array of all friends: ', friendList);
